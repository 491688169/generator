...
$R.add('service', 'inspector', {
    ...
    isInlineTag: function(tag, extend)
    {
        ...
        var tagTran
        try{
            tagTran = tag.toLowerCase()
        }catch(err){
            tagTran = undefined
        }
        return (this._isTag(tag) && tags.indexOf(tagTran) !== -1);
    },
    ...
    _set_align: function(align)
    {
        ...
        else
        {
            ...
            switch (align)
                ...
                // 自定义6：文字图片环绕问题
                // $el.css({ 'float': imageFloat, 'margin': imageMargin });
                $el.css({'text-align': textAlign, 'background': 'transparent' });
                $el.attr('rel', $el.attr('style'));
        }
    },
